...
def predictor(input_dir=None,
    ...
    Returns
    -------- 
    ./output_dir/X_prediction_results.csv: A table containing all the detection, and picking results. Duplicated events are already removed.      
    ...
    if args['input_dir']:
        ...
        if platform.system() == 'Windows': 
            station_list = [ev.split(".")[0] for ev in listdir(args["input_dir"]) ];  # input_dir = ../hdf5_merged/     station_list= [QJ022022_denoised,..]
        station_list = sorted(set(station_list))[6:7]   # set去重，sort默认升序排列
        ...
        for ct, st in enumerate(station_list):
            ...
            result_csvgen = open(os.path.join(save_dir,'picks_results.csv'), 'w')          
            predict_writer = csv.writer(result_csvgen, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
            predict_writer.writerow([   'station_id'
                                        'waveform_id',
                                        'phase_time',
                                        'phase_type',
                                        'phase_score',
            ...
            prediction_list = list_hdf5_groups(fl)  # ['QJ.QJ02.20220823.090001', 'QJ.QJ02.20220823.090002', 'QJ.QJ02.20220823.090003',...]
            # 计算初始时间
            #print(prediction_list)
            # 原始语句如下，如果是已经编码好顺序的，就使用下面这句原始语句
            _, _, date_str, time_str = sorted(set(prediction_list))[0].split(".")
            hour_str = time_str[:2]
            start_time_str = f"{date_str[:4]}/{date_str[4:6]}/{date_str[6:]} {hour_str}:00:00"
            start_time = UTCDateTime(start_time_str)   # 每个QJ022022文件开始的时间
            ...
            for _ in range(total_iters):
                ...
                for ts in range(pred_DD_mean.shape[0]):  # 遍历一个batch ts=0，。。。，199
                    ...
                    dataset = pred_set[evi]    # 要预测的单条数据
                    
                    _, _, _, counts_str = evi.split(".")
                    counts = int(counts_str[2:])  
                    # sensitivity = 5.03e8   # 根据实际情况调整
                    amp = np.max(np.abs(dataset), axis=-1)  ## 找到三个通道中最大的绝对值，（20000，）维
                    # amp_physical = amp / sensitivity  # 单位：m/s
                    ...
                    _results_writer(st,evi,start_time,counts, amp, predict_writer, result_csvgen, matches)     # "station_id", "waveform_id","phase_time", "phase_type", "phase_score", "phase_amplitude"
            ...
            dd = pd.read_csv(os.path.join(save_dir,'picks_results.csv'))
            print(f'\n', flush=True)
        ...
        # 没有input_dir,直接读取input_hdf5.predict for DiTing2,predict单个文件
        ...
        # hour_str = time_str[:2]
        # start_time_str = f"{date_str[:4]}/{date_str[4:6]}/{date_str[6:]} {hour_str}:00:00"
        # start_waveform_index = int(time_str[2:])    # 如果编码不是从0开始的，用这句，否则注释掉
        # start_time = UTCDateTime(start_time_str)   # 每个QJ022022文件开始的时间
        # ipdb.set_trace()
        ...
        for bn in range(total_iters): 
            ...
            pred_set={}
            for ID in new_list:
                dataset = fl.get('earthquake/'+str(ID))   # (20000,)
                pred_set.update( {str(ID) : dataset})
            for ts in range(pred_DD_mean.shape[0]): 
                ...
                _, _, date_str, counts_str = evi.split(".")
                counts = int(counts_str[2:])-1 # 原始已编码好的顺序的话只用这句
                hour_ind = int(counts_str[:2])
                start_time= UTCDateTime(f"{date_str[:4]}/{date_str[4:6]}/{date_str[6:]} {hour_ind}:00:00")
                # counts = int(counts_str[2:])-start_waveform_index+1   # 该波形是该文件第几个波形
                # sensitivity = 5.03e8   # 根据实际情况调整
                amp = np.max(np.abs(dataset), axis=-1)  ## 找到三个通道中最大的绝对值，（20000，）维
                # amp_physical = amp / sensitivity  # 单位：m/s
                matches=picker_prediction(args, pred_DD_mean[ts], pred_PP_mean[ts], pred_SS_mean[ts],
                ...
                st = evi.split(".")[1]   # station id
                # _results_writer( evi, predict_writer, result_csvgen, matches) 
                _results_writer_debug(st,evi,start_time,counts, amp, predict_writer, result_csvgen, matches) 
...
def list_hdf5_groups(hdf5_file):
    ...
    group_list = hdf5_file['earthquake'].keys()    # 为了巧家和谛听1准备的earthquake组名
    #group_list = hdf5_file.keys()
...
def _results_writer( 
    ...
    Writes the prediction resultss into a CSV file. Prepare for association task

    Parameters
    ----------

    station_id: str
        The index of station. eg:['QJ01'.'QJ02']    
    
    evi:str
        The title or name of waveform
        
    output_writer: obj
        Write the detection/picking results in the CSV file.
        
    csvfile: obj
        The CSV file contains picks.  
    ...
    wave_time = start_time + waveform_duration*counts
    delta = 500
    if P_pred:
        
        converted_P = wave_time + P_pred/100
        subset = amp[P_pred : P_pred + delta * 3]
        P_phase_amp = np.max(subset).item() if subset.size > 0 else np.max(amp[P_pred : P_pred + delta * 3]).item()  # 默认振幅为 0  ## peak amplitude
    ...
    if S_pred:
        ...
        converted_S = wave_time + S_pred/100
        S_phase_amp = np.max(amp[S_pred : min(S_pred + delta * 3, 20000)]).item()
    else:
        S_phase_amp = None
        converted_S = None
...
def _plotter( dataset, evi, args, save_figs, yh1, yh2, yh3, yh1_std, yh2_std, yh3_std, matches):
    ...
    # 第一个图     
    axs[0].plot(data[:, 0], color='#6E4B47')
    ...
    if len(predicted_P) > 0: 
        ...
        for pt in predicted_P:
            if pt:
                ppl = axs[0].axvline(int(pt), color='#6B8E23', linestyle='--', linewidth=1.5,  label='Predicted_P_Arrival')

    if len(predicted_S) > 0:  
        for st in predicted_S: 
            if st:
                ssl = axs[0].axvline(int(st), color='#DAA520', linestyle='--', linewidth=1.5,  label='Predicted_S_Arrival')
    if ppl or ssl:    
        axs[0].legend(loc='upper right', fontsize=9) 

    # 4行1列第2个图  
    axs[1].plot(data[:, 1] , color='#6E4B47')
    ...
    if len(predicted_P) > 0: 
        ...
        for pt in predicted_P:
            if pt:
                ppl = axs[1].axvline(int(pt), color='#6B8E23', linestyle='--', linewidth=1.5,  label='Predicted_P_Arrival')


    if len(predicted_S) > 0:  
        for st in predicted_S: 
            if st:
                ssl = axs[1].axvline(int(st), color='#DAA520', linestyle='--', linewidth=1.5,  label='Predicted_S_Arrival')
    ...
    if ppl or ssl:    
      ...
      # 第3个预测图
    axs[2].plot(data[:, 2], color='#6E4B47')   
    #plt.tight_layout() 
    ...
    if len(predicted_P) > 0: 
        ...
        for pt in predicted_P:
            if pt:
               ppl = axs[2].axvline(int(pt), color='#6B8E23', linestyle='--', linewidth=1.5,  label='Predicted_P_Arrival')

    if len(predicted_S) > 0:  
        for st in predicted_S: 
            if st:
                ssl = axs[2].axvline(int(st), color='#DAA520', linestyle='--', linewidth=1.5,  label='Predicted_S_Arrival')
    ...
    if args['estimate_uncertainty']:                               
        axs[3].plot(x, yh1, color='#6E4B47', linestyle='--', linewidth=1.5, label='Detection')    # x(1,) yh1(20000,)
        lowerD = yh1-yh1_std
        upperD = yh1+yh1_std
        axs[3].fill_between(x, lowerD, upperD, color='#6E4B47', alpha=0.2)    
        
             
    else:
        axs[3].plot(x, yh1, color='#6E4B47', linestyle='--', alpha = 0.5, linewidth=1.5, label='Detection') 
    ...
    axs[3].plot(x, yh2, color='#6B8E23', linestyle='-.', linewidth=1.5, label='P_probability')
    axs[3].plot(x, yh3, color='#DAA520',linestyle='dotted', linewidth=1.5, label='S_probability')
    #plt.tight_layout()       
    #axs[3].ylim((-0.1, 1.1))
