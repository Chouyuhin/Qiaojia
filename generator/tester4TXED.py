...
def tester(input_hdf5=None,
    ...
    test=[ii for ii in test if ii.split("_")[-1]=='EV']
    ...
    for _ in range(int(np.ceil(len(test) / args['batch_size']))):
        ...
        for ID in new_list:   # list of keys for tests
            ...
            test_set.update( {str(ID) : data}) 
            metaP_test_set.update( {str(ID) : spt})    
            metaS_test_set.update( {str(ID) : sst})
        ...
        for ts in range(pred_DD_mean.shape[0]):  # 遍历一个batch ts=0，。。。，199
            ...
            spt =  metaP_test_set[evi]
            sst = metaS_test_set[evi]
...
def _plotter(spt,sst, dataset, evi, args, save_figs, yh1, yh2, yh3, yh1_std, yh2_std, yh3_std, matches):
    ...
    #  4行1列，第一行     
    axs[0].plot(data[:6000, 0], color='#1f77b4')
    ...
    axs[1].plot(data[:6000, 1] , color='#1f77b4')  # 4行1列第2个图  
    # plt.plot(data[:, 1] , 'k')
    ...
    axs[2].plot(data[:6000, 2], color='#1f77b4')    # 第3个预测图
    # plt.plot(data[:, 2], 'k')   
    # plt.tight_layout() 
    #ipdb.set_trace()              
    ...
    if args['estimate_uncertainty']:                               
        axs[3].plot(x[:6000], yh1[:6000], color='#1f77b4', linestyle='--', linewidth=1.5, label='Detection')    # x(1,) yh1(20000,)
        lowerD = yh1-yh1_std
        upperD = yh1+yh1_std
        plt.fill_between(x[:6000], lowerD[:6000], upperD[:6000], alpha=0.2, color='#1f77b4')            
        ...
        axs[3].plot(x[:6000], yh1[:6000], color='#1f77b4', linestyle='--',  linewidth=1.5, label='Detection')                       
    ...
    axs[3].plot(x[:6000], yh2[:6000], color='#d62728', linestyle='-.', linewidth=1.5, label='P_probability')
    axs[3].plot(x[:6000], yh3[:6000], color='#ff7f0e', linestyle='dotted', linewidth=1.5, label='S_probability')
    # plt.tight_layout()       
    # plt.ylim((-0.1, 1.1))
