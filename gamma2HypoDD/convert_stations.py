...
for i in tqdm(range(len(stations))):
    ...
    network_code, station_code = stations.iloc[i]['station_id'][:2],stations.iloc[i]['station_id'][2:]    # QJ,01
    station_weight = " "
    ...
    line_hypoinverse = f"{station_code:<5} {network_code:<2} {station_weight}{abs(lat_degree):2.0f} {abs(lat_minute):7.4f}{north}{abs(lng_degree):3.0f} {abs(lng_minute):7.4f}{west}{elevation:4.0f}\n"
