...
def download_hypodd():
    ...
    '''
    #os.system("curl -O http://www.ldeo.columbia.edu/~felixw/HYPODD/HYPODD_1.3.tar.gz")
    #os.system("tar -zxvf HYPODD_1.3.tar.gz")
    # if os.path.exists("g77"):
    #     os.remove("g77")
    # if os.path.exists("f77"):
    #     os.remove("f77")
        
    # os.system('cmd //c mklink f77 C:\\mingw64\\bin\\gfortran')
    # os.system("cmd //c mklink g77 C:\\mingw64\\bin\\gfortran")
...
def convert_picks():
    ...
    MAXEVENT = len(events) // ((len(events) - 1) // MAXEVENT + 1) + 1  #10000
    ...
    for i in tqdm(range(len(events))):
        ...
        for j in picks_idx:
            pick = picks.iloc[j]
            network_code, station_code = pick['id'][:2],pick['id'][2:]
...
def run_hypoDD(data_path):
    ...
    ph2dt_path = os.path.abspath("../HYPODD/src/ph2dt/ph2dt")
    #PH2DT_CMD = f" ../HYPODD/src/ph2dt/ph2dt ph2dt.inp"
    
    hypoDD_path = os.path.abspath("../HYPODD/src/hypoDD/hypoDD")
    #HYPODD_CMD = f"cd {data_path} && ./HYPODD/src/hypoDD/hypoDD hypoDD.inp"
    
    #if os.system(PH2DT_CMD) != 0:
    if os.system(f"{ph2dt_path} ph2dt.inp") != 0:
        raise RuntimeError(f"{ph2dt_path}" + " failed!")
    #if os.system(HYPODD_CMD) != 0:
    print(os.getcwd())
    if os.system(f"{hypoDD_path} hypoDD.inp") != 0:
        raise RuntimeError(f"{hypoDD_path}" + " failed!")
    
    
    idx = data_path.split('/')[-1].lstrip('tmp_')
    
    os.system(f"cp hypoDD.reloc {current_dir}/hypoDD_{idx}.reloc")
    os.system(f"cp dt.ct {current_dir}/dt_{idx}.ct")
    os.system(f"cp event.dat {current_dir}/event_{idx}.dat")
    os.system(f"cp event.sel {current_dir}/event_{idx}.sel")
...
if __name__ == "__main__":
    ...
    print(tmp_paths)
    with multiprocessing.Pool(processes=len(tmp_paths)) as pool:
